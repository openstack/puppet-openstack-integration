...
# Cinder Backup
echo "VolumesBackupsAdminTest" >> /tmp/openstack/tempest/test-whitelist.txt
...
if [ "${TEMPEST_FROM_SOURCE}" = true ]; then
    ...
    run_tempest/bin/pip3 install -c https://opendev.org/openstack/requirements/raw/branch/stable/victoria/upper-constraints.txt -U -r requirements.txt
